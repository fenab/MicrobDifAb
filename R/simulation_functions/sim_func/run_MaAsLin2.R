#############################################
# Vanilla LM with Library Size as Covariate # replaced by log (lib sc.)
#############################################
...
fit.MaAsLin2 <- function(features, metadata, libSize, ID, transformation, MultTestCorrection){
  ...
  paras <- pbapply::pbsapply(1:ncol(features), simplify=FALSE, function(x){
    ...
    if(sum(featuresVector!=0)<1){
      ...
      # Fit Model
      dat_sub <- data.frame(expr = as.numeric(featuresVector), metadata, log(libSize+1), ID)
  ...
  paras$relevance<- (paras$qval< 0.05)*1
...
list.MaAsLin2<-function(physeq, transformation='LOG', MultTestCorrection = "BH"){
  foreach(physeq=physeq, .export=c("fit.MaAsLin2", "AST", "LOG", "LOGIT"),
          .packages=c("dplyr", "pbapply", "car", "nlme"), .errorhandling = 'remove') %dopar% 
  {
    ...
    ID<-physeq$ID;
    DD<-fit.MaAsLin2(features, metadata, libSize, ID, transformation, MultTestCorrection)
...
#physeq=fitlist
#list.MaAsLin2(physeq, transformation='LOG', MultTestCorrection = "BH")
